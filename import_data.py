...
# ----------------------------------------------------------
#  Below is downloading data & constructing the shallow water class object
# ----------------------------------------------------------
...
download_url = "https://darus.uni-stuttgart.de/api/access/datafile/133021"
...
# -----------------------------
# 2. Build the params object expected by ShallowWater2D
# -----------------------------

params = SimpleNamespace()

# Runtime / distributed settings
params.num_workers = 0
params.num_workers_eval = 0
params.local_rank = 0
params.global_rank = 0
params.n_gpu_per_node = 1
params.test_seed = 42
params.overfit_test = False

# Augmentation settings
params.noise = 0.0
params.noise_type = "additive"
params.flip = False
params.rotate = False

# Optional outputs
params.use_raw_time = True

# Symbol settings
params.symbol = SimpleNamespace()
params.symbol.symbol_input = False

# Data settings
params.data = SimpleNamespace()
params.data.t_num = 10          # number of time steps returned per sample
params.data.x_num = 64          # spatial resolution returned per sample
params.data.tie_fields = True   # keep original data channels
params.data.max_output_dimension = 1

params.data.train_val_test_ratio = [0.8, 0.1, 0.1]

params.data.random_start = {
    "train": True,
    "val": False,
    "test": False,
    "start_max": -1,
...
# Dataset-specific shallow-water settings
params.data.shallow_water = SimpleNamespace()
params.data.shallow_water.t_step = 1
params.data.shallow_water.x_num = 128
params.data.shallow_water.data_path = local_path
params.data.shallow_water.c_mask = [1]


# -----------------------------
# 3. Construct the dataset object
# -----------------------------

symbol_env = None

shallow_water_dataset = ShallowWater2D(
    params=params,
...
for sample_idx, sample in enumerate(islice(shallow_water_dataset, num_samples_to_show)):
    print(f"\nSample {sample_idx}")
