...
params = types.SimpleNamespace(
    ...
    data=types.SimpleNamespace(
        ...
        t_num=26,#101,# Other systems (e.g. NS) have different time steps... (Reduced from 101 for performance)
...
try:
    ...
    start_epoch = 0
    print(f"Checkpoint file not found at {checkpoint_path}. Starting training from epoch {start_epoch}.")
except RuntimeError as e:
    start_epoch = 0
    print(f"Error loading checkpoint due to model parameter shape mismatch: {e}. Starting training from epoch {start_epoch} (model not loaded).")
except Exception as d:
    start_epoch = 0
    print(f"Error loading checkpoint: {d}. Starting training from epoch {start_epoch}.")
# finally:
...
# Calculate mean and std for data normalization from the training set
# This should ideally be done once over the entire training dataset.
# For simplicity, we'll calculate it from a large batch or the first few batches.
# In a real scenario, you'd calculate this over the full training data.
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            input_times = times[:, :-1]
            input_times = input_times.unsqueeze(-1) # Ensure correct shape for validation as well
...
# Plotting the loss curves and saving to CSV
...
plt.legend()
plt.grid(True)
plt.savefig('training_validation_loss_plot.png') # Add this line to save the plot
# plt.show() # disabled since one has to close the window to proceed.

# Saving the loss values to a CSV file
import pandas as pd
...
loss_df = pd.DataFrame({
    ...
    'Block size': params.patch_size,
    'Embedding dimension': params.dim,
    'feed forward': params.dim_feedforward,
    'Epoch': range(len(train_losses)),
    'Train Loss': train_losses,
    'Validation Loss': val_losses
})
loss_df.to_csv('training_validation_losses.csv', index=False)
print("Training and validation losses saved to 'training_validation_losses.csv'")
...
ground_truth_sequence = data_sample[:, input_len + initial_time:, :, :, :]
...
fig, axes = plt.subplots(num_frames_to_plot, 3, figsize=(10, num_frames_to_plot * 5))
...
plt.tight_layout(rect=[0.3, 0.03, 0.3, 0.9]) # [left, bottom, right, top] margins
plt.show()



from conservation import plot_mass, plot_mass_der
